...
```bash
# Contracts — detect foundry root (SE2 monorepo or standalone)
if [ -f project/packages/foundry/foundry.toml ]; then cd project/packages/foundry
elif [ -f project/foundry.toml ]; then cd project
elif [ -f project/contracts/foundry.toml ]; then cd project/contracts
fi
forge build
forge test -vv

# Frontend (SE2 monorepo)
cd project && yarn next:build
```
...
If `project/packages/nextjs/` does NOT exist, report "Frontend: NOT BUILT" and skip all frontend checks below. Do not report "yarn next:build: PASS" for a default scaffold with no dApp code.
...
**Provider setup (SE2):**
- `scaffold.config.ts` at project root configures `targetNetworks` — should include `chains.foundry` or `chains.hardhat` (chain ID 31337) for local dev
- `packages/nextjs/app/layout.tsx` is a Server Component (no `"use client"`) — providers are handled by SE2's `ScaffoldEthAppWithProviders`

**Contract wiring (SE2):**
- `packages/nextjs/contracts/deployedContracts.ts` exists and is auto-generated by `yarn deploy`
- Components use scaffold hooks (`useScaffoldReadContract`, `useScaffoldWriteContract`) — NOT raw wagmi hooks
- No hardcoded contract addresses in components (scaffold hooks auto-wire from deployedContracts)

**Environment:**
- `.env.local` or `.env.example` in `packages/nextjs/` (if needed beyond SE2 defaults)

**Code quality:**
- `formatUnits` / `parseUnits` use correct decimals for each token
- No raw `useReadContract` / `useWriteContract` — scaffold hooks should be used instead
...
**Transaction UX:**
- `useScaffoldWriteContract` handles receipts automatically (no need for separate `useWaitForTransactionReceipt`)
