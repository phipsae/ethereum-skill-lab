...
# ─── Config ───────────────────────────────────────────────────────────────────
PROJECT_ID="${1:-erc20-staking}"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SKILL_DIR="$(cd "$SCRIPT_DIR/../ethereum-building-skill" && pwd)"
BASE_DIR="$SCRIPT_DIR"
...
trap 'rm -f "$BUILDER_PROMPT_FILE" "$REVIEWER_PROMPT_FILE"; cleanup' EXIT
...
BUILDER_END=$(ts)
log "Builder finished (exit code: $BUILDER_EXIT)"

# Fallback: if builder didn't write a report, create one from stdout
if [[ ! -f "$BUILD_DIR/builder-report.md" ]]; then
  log "No builder report found — generating fallback from stdout"
  {
...
REVIEWER_END=$(ts)
log "Reviewer finished (exit code: $REVIEWER_EXIT)"

# Fallback reviewer report
if [[ ! -f "$BUILD_DIR/reviewer-report.md" ]]; then
  log "No reviewer report found — generating fallback from stdout"
  {
...
log "  Latest:   $REPORTS_DIR/latest.md"
